    ...
    return folders
      ...
      .map(folder => (
        <div key={folder.id} style={{ marginLeft: `${level * 16}px` }}>
          <div 
            className={`flex items-center p-2 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors touch-feedback ${
          ...
          >
            <button 
              ...
              className="mr-1 p-1 hover:bg-gray-200 rounded transition-colors touch-target"
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-gray-100 overflow-hidden relative">
      ...
      <AnimatePresence>
        {!sidebarCollapsed && (isMobile || isTablet) && (
          <motion.div 
            ...
            className="fixed inset-0 bg-black bg-opacity-50 z-30 backdrop-blur-sm mobile-overlay"
      ...
      <motion.div 
        initial={false}
        animate={{ 
          x: sidebarCollapsed && (isMobile || isTablet) ? -320 : 0,
          width: sidebarCollapsed ? (isMobile || isTablet ? 0 : 64) : 320
        }}
        ...
        className={`bg-white border-r border-gray-200/80 flex flex-col shadow-lg backdrop-blur-sm z-40 relative overflow-hidden ${
          isMobile || isTablet ? 'fixed h-full' : 'relative'
        }`}
      >
        {/* Sidebar Header */}
        <div className="p-3 sm:p-4 lg:p-6 border-b border-gray-200/60 bg-white/95 backdrop-blur-sm flex-shrink-0 safe-area-top">
            ...
            {!sidebarCollapsed && (
              ...
              >
                ...
                >
                  ...
                  <span className={isMobile ? '' : ''}>New Note</span>
                </button>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    <span className={isMobile ? 'hidden' : ''}>New Folder</span>
                  </button>
          ...
          {!sidebarCollapsed && (
            <>
              ...
              <motion.div 
                ...
                exit={{ opacity: 0, y: -10 }}
                transition={{ delay: 0.2 }}
                className="flex-1 overflow-y-auto scrollbar-thin scroll-optimized safe-area-bottom"
              >
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-w-0 overflow-hidden">
        ...
        <motion.div 
          initial={false}
          animate={{ 
            x: selectedNote && isEditing && (isMobile || isTablet) ? -100 : 0,
            opacity: selectedNote && isEditing && (isMobile || isTablet) ? 0 : 1
          }}
          transition={{ duration: 0.3, ease: [0.4, 0, 0.2, 1] }}
          className={`${
            selectedNote && isEditing && (isMobile || isTablet) ? 'hidden' : 'flex'
          } w-full lg:w-80 xl:w-96 bg-white border-r border-gray-200/60 flex-col shadow-lg`}
        >
          <div className="p-3 sm:p-4 lg:p-6 border-b border-gray-200/60 bg-white/95 backdrop-blur-sm flex-shrink-0 safe-area-top">
            <div className="flex items-center justify-between">
          ...
          <div className="flex-1 overflow-y-auto scrollbar-thin bg-gradient-to-b from-white to-gray-50/30 scroll-optimized safe-area-bottom">
            <AnimatePresence mode="wait">
              {filteredNotes.length === 0 ? (
                <motion.div 
                  ...
                  className="p-4 sm:p-6 lg:p-8 text-center text-gray-500 mobile-spacing"
                >
                  <BookOpen className="h-10 w-10 sm:h-12 sm:w-12 lg:h-16 lg:w-16 mx-auto mb-4 text-gray-300" />
                  <p className="mb-2 font-semibold text-gray-600 text-sm sm:text-base lg:text-lg responsive-text-lg">No notes found</p>
                  <p className="text-xs sm:text-sm lg:text-base text-gray-500 mb-4 sm:mb-6 responsive-text-base">Create your first note to get started</p>
                  <button
                    onClick={() => setShowNewNoteModal(true)}
                ...
                <motion.div 
                  ...
                  className={
                    viewMode === 'grid' 
                      ? 'p-3 sm:p-4 lg:p-6 grid grid-cols-1 gap-2 sm:gap-3 lg:gap-4 mobile-grid' 
                      : 'space-y-1'
                  }
                >
                  {filteredNotes.map((note, index) => (
                    <motion.div
                      ...
                      className={`note-card note-card-mobile group ${
                        viewMode === 'grid' 
                          ? 'p-3 sm:p-4 lg:p-5 rounded-xl min-h-[120px] sm:min-h-[140px] lg:min-h-[160px] shadow-sm hover:shadow-lg' 
                          : 'p-3 sm:p-4 lg:p-5'
                      } cursor-pointer border-l-4 hover:bg-gray-50/80 transition-all duration-300 gpu-accelerated touch-feedback ${
        ...
        {/* Editor */}
        <div className="flex-1 flex flex-col bg-white min-w-0 shadow-sm overflow-hidden">
          {selectedNote ? (
            <>
              {/* Editor Header */}
              <div className="p-3 sm:p-4 border-b border-gray-200 bg-white flex-shrink-0 safe-area-top">
                <div className="flex items-center justify-between mb-3 sm:mb-4 gap-3 sm:gap-4">
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-2 lg:hidden">
                      {sidebarCollapsed && (
                        <button
                          ...
                          className="touch-target p-2 hover:bg-gray-100 rounded transition-colors focus-visible-ring"
                          title="Open sidebar"
                        >
                    ...
                    {isEditing ? (
                      <input
                        ...
                        className="text-lg sm:text-xl font-bold bg-transparent border-none outline-none text-gray-900 placeholder-gray-500 min-w-0 flex-1 focus:ring-2 focus:ring-blue-500 rounded px-2 py-1 responsive-text-xl"
                        placeholder="Note title..."
                      />
                    ) : (
                      <h1 className="text-lg sm:text-xl font-bold text-gray-900 truncate responsive-text-xl">{selectedNote.title}</h1>
                    )}
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                    {/* Color Picker - Responsive visibility */}
                    <div className={`${isEditing ? 'hidden sm:flex' : 'flex'} items-center gap-1 overflow-x-auto max-w-[200px] scrollbar-thin`}>
                      {colorPalette.map(color => (
                        <button
                          key={color}
                          onClick={() => setCurrentColor(color)}
                          className={`color-picker-button w-5 h-5 sm:w-6 sm:h-6 rounded border-2 transition-all hover:scale-110 focus-visible-ring flex-shrink-0 ${
                            currentColor === color ? 'border-gray-400 shadow-sm' : 'border-gray-200'
                          }`}
                      ...
                      {isEditing ? (
                        <>
                          <button
                            ...
                            className="flex items-center gap-1 px-2 sm:px-3 py-1 text-xs sm:text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-gray-700 focus-visible-ring touch-target"
                          >
                            {editorMode === 'edit' ? <Eye className="h-3 w-3 sm:h-4 sm:w-4" /> : <Edit className="h-3 w-3 sm:h-4 sm:w-4" />}
                            <span className="hidden sm:inline">{editorMode === 'edit' ? 'Preview' : 'Edit'}</span>
                          </button>
                          <button
                            onClick={saveNote}
                            className="flex items-center gap-1 px-2 sm:px-3 py-1 text-xs sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium focus-visible-ring shadow-sm touch-target"
                          >
                          ...
                          <button
                            ...
                            className="touch-target p-1 hover:bg-gray-100 rounded transition-colors focus-visible-ring"
                ...
                {isEditing && editorMode === 'edit' && (
                  <div className="flex items-center gap-1 p-2 bg-gray-50 rounded-lg overflow-x-auto scrollbar-thin border toolbar-mobile">
                    {toolbarItems.map((item, index) => (
                      <button
                        key={index}
                        onClick={item.action}
                        className="toolbar-button toolbar-button-mobile p-2 hover:bg-gray-200 rounded transition-all flex-shrink-0 focus-visible-ring touch-target"
                        title={item.tooltip}
                      >
                        <item.icon className="h-3 w-3 sm:h-4 sm:w-4 text-gray-700" />
                      </button>
                    ))}
                    <div className="w-px h-6 bg-gray-300 mx-2 flex-shrink-0" />
                    <button
                      onClick={() => insertText('$$', '$$')}
                      className="px-2 py-1 text-xs sm:text-sm bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors font-medium flex-shrink-0 focus-visible-ring"
                      title="LaTeX Block"
                    >
                      LaTeX
                    </button>
              ...
              {/* Editor Content */}
              <div className="flex-1 p-3 sm:p-4 overflow-hidden keyboard-adjust" style={{ 
                ...
                {isEditing ? (
                  editorMode === 'edit' ? (
                    <textarea
                      ...
                      className="w-full h-full resize-none border-none outline-none bg-transparent font-mono text-xs sm:text-sm text-gray-900 placeholder-gray-500 leading-relaxed focus:ring-2 focus:ring-blue-500 rounded p-2 responsive-text-base"
                      placeholder="Start writing your note... Use $...$ for inline LaTeX or $$...$$ for display LaTeX."
                    />
                  ) : (
                    <div className="h-full overflow-y-auto scrollbar-thin animate-fade-in scroll-optimized">
                      {renderMarkdown(editContent)}
                    </div>
                  )
                ) : (
                  <div className="h-full overflow-y-auto scrollbar-thin scroll-optimized">
              ...
              <div className="p-3 sm:p-4 border-t border-gray-200 bg-gray-50 flex-shrink-0 safe-area-bottom">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs sm:text-sm text-gray-600">
                  <div className="flex flex-wrap items-center gap-2 sm:gap-4">
                    ...
                    <span className="hidden sm:inline">Words: {selectedNote.content.split(' ').filter(word => word.length > 0).length}</span>
                    <span className="hidden md:inline">Characters: {selectedNote.content.length}</span>
                  </div>
                  
                  <div className="flex items-center gap-1 sm:gap-2 flex-wrap">
                    {selectedNote.tags.map(tag => (
                      <span key={tag} className="tag px-2 py-1 bg-gray-200 text-gray-700 text-xs rounded font-medium">
                        #{tag}
                      </span>
                    ))}
                  </div>
            ...
            <div className="flex-1 flex items-center justify-center text-gray-500 p-4 md:p-8 animate-fade-in safe-area-inset">
              <div className="text-center max-w-md mobile-spacing">
                <BookOpen className="h-10 w-10 sm:h-12 sm:w-12 md:h-16 md:w-16 mx-auto mb-4 text-gray-300" />
                <h3 className="text-base sm:text-lg font-medium mb-2 text-gray-700 responsive-text-lg">Select a note to start</h3>
                <p className="text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6 responsive-text-base">Choose a note from the sidebar or create a new one to begin writing</p>
                <button
                  onClick={() => setShowNewNoteModal(true)}
                  className="inline-flex items-center gap-2 bg-gray-900 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-800 transition-all duration-200 font-medium shadow-sm hover:shadow-md transform hover:-translate-y-0.5 text-xs sm:text-sm touch-feedback"
                >
      ...
      <AnimatePresence>
        {showNewNoteModal && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-3 sm:p-4 safe-area-inset"
